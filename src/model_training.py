...
class ChurnModelTrainer:
    ...
    def train_all_models(self, X_train, y_train, X_val, y_val):
        ...
        for model_name, model in self.models_dict.items():
            try:
                pipeline, train_time = self.train_model(model, X_train, y_train, model_name)

                results = self.evaluate_model(pipeline, X_train, y_train, X_val, y_val, model_name)
    ...
    def cross_validate(self, model_name, X, y, cv=5, scoring="roc_auc"):
        ...
        scores = cross_val_score(pipeline, X, y, cv=cv_strategy, scoring=scoring, n_jobs=-1)
...
class ModelOptimizer:
    ...
    def optimize(self, X_train, y_train, search_type="random"):
        ...
        if search_type == "random":
            ...
            search = GridSearchCV(
                estimator=self.model, param_grid=self.param_grid, cv=self.cv, scoring=self.scoring, n_jobs=-1, verbose=1
            )
...
def get_default_models():
    ...
    try:
        ...
        models["XGBoost"] = xgb.XGBClassifier(
            n_estimators=100, learning_rate=0.1, random_state=42, n_jobs=-1, eval_metric="logloss"
        )
    except ImportError:
        pass
...
def get_param_grids():
    ...
    param_grids = {
        ...
        "CatBoost": {"iterations": [100, 200, 300], "learning_rate": [0.01, 0.05, 0.1], "depth": [4, 6, 8]},
    }
